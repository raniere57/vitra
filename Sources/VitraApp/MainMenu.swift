...
enum MainMenu {
    static func build(
        ...
        let toggleCodex = viewMenu.addItem(
            withTitle: "Codex Sessions",
            action: #selector(AppDelegate.toggleCodexSidebar(_:)),
            keyEquivalent: key("codex_sidebar", keybindings)
        )
        toggleCodex.keyEquivalentModifierMask = [.command, .option]
